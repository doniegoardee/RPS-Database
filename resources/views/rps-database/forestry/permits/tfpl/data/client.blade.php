...
<div class="container-fluid d-flex flex-column" style="height: 100vh; overflow: hidden;">
    ...
    <div class="flex-grow-1 overflow-auto">
        ...
        <div class="mb-4">
            <div class="d-flex align-items-center mb-2">
                <a href="{{ route('tfpl') }}" class="btn btn-sm btn-primary shadow-sm me-3">
    ...
    @endif
     ...
     <div class="d-flex justify-content-between flex-wrap gap-2 mt-3">
        ...
        <a href="{{ route('tfpl.template') }}" class="btn btn-sm btn-success shadow-sm ms-auto" >
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('client.tfpl',['add' => $add->address]) }}" class="btn btn-danger btn-sm shadow-sm" target="_blank">
                    <i class="fa-solid fa-chart-simple me-1"></i> Generate Pdf Report
                </a>


                <a href="{{ route('tfpl.address', ['address' => $add->address]) }}" class="btn btn-sm btn-success shadow-sm">
...
<div id="clientList" style="display: flex; flex-direction: column; gap: 0.5rem;">
    ...
    <a
        href="{{ route('tfpl.client-data',  $item->id) }}"
        class="client-item"
        style="
            display: flex;
            align-items: center;
...
<div class="modal fade" id="addClientModal" tabindex="-1" aria-labelledby="addClientModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('add-client.tfpl',[ 'address'=> $add->address]) }}" id="Client" method="POST">
                @csrf
...
<div class="modal fade" id="ImportClientModal" tabindex="-1" aria-labelledby="ImportClientModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form action="{{ route('tfpl.import',['add' => $add->address]) }}" method="POST" enctype="multipart/form-data">
      @csrf
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="ImportClientModalLabel">Import Transport Finish Product Lumber Excel File</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
