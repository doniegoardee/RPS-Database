...
public function index(){
    ...
    $address = Address::where('type','SP')->get();

    return view('rps-database.lands.sp.index',compact('address'));
...
public function client($address){
        ...
        $client = LandsParents::where('address', $address)->where('type','SP')
            ->orderBy('name','asc')
            ->get();

        return view('rps-database.lands.sp.data.client',compact('add','client'));
}
