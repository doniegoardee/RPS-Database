...
public function index(){
    ...
    $address = Address::where('type','RFPA')->get();

    return view('rps-database.lands.rfpa.index',compact('address'));
...
public function client($address){
        ...
        $client = LandsParents::where('address', $address)->where('type','RFPA')
            ->orderBy('name','asc')
            ->get();

        return view('rps-database.lands.rfpa.data.client',compact('add','client'));
}
...
public function client_data($name){
        ...
        return view('rps-database.lands.rfpa.data.data', compact('client', 'parent', 'table'));
    }
